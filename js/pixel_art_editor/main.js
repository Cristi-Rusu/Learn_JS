...
// creates a line, by iterating on a reference axis and
...
// returns the array of pixels to draw
function linePxArray(to, state, start) {
...
function line(start, state, dispatch) {
    function drawLine(to) {
        const drawn = linePxArray(to, state, start);
...
function draw(start, state, dispatch) {
    function drawPixels(pos, currState) {
        let drawn = [];
        // if 'pos' is right next to 'start'(including diagonally)
        // draw a single pixel
        if (pos.x >= start.x - 1
            && pos.x <= start.x + 1
            && pos.y >= start.y - 1
